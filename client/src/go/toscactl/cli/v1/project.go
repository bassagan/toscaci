...
var (
	...
	createCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			dbtype := __0.DBType(strings.ToUpper(args[0]))
			projectName := args[1]

			templateType := __0.CreateEmpty
			if fromConnectionString != "" {
				templateType = __0.CreateFromDatabase
			} else if fromDefinition != "" {
				templateType = __0.CreatefromDefinition
			}

			toscaProvider, err := __0.NewProvider(appConfig)
			if err != nil {
				log.Panic(err)
			}

			createRequest := __0.ProjectCreateRequest{
